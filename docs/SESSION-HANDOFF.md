...
- **On-chain `IdentityRegistry.verifyAddress` real**: `lib/server/chain/identityRegistry.ts` con viem directo a Fuji, idempotente (lee `isVerified` antes de escribir). **Nota post-merge**: el comentario JSDoc del archivo dice "bypassea el SDK porque tira throw" — eso era cierto al momento de escribirlo pero el adapter avalanche del SDK ya está funcional (commit d71c7e6). Es deuda técnica: refactor a `chainClient.identity.registerIdentity()` para evitar duplicación.
...
- **UI Governance Idea 2.2 cerrada visualmente** (sesión 3): `useGovernance` hook (multicall `useReadContracts` para `proposals(i)` + `getProposalStatus(i)` + per-holder `votingPower`/`hasVoted`/`voteOf`); `CreateProposalPanel` issuer (descripción + datetime-local deadline + cap-table autoload + tabla editable de pesos + KYC soft-warn + capture revert `NotIssuerAdmin`); `VotesPendingBanner` en dashboard investor; `/issuer/governance` con tabla de propuestas + botón Finalizar contextual; `/investor/governance` lista de cards con 3 botones de voto (Sí/No/Abstención); `/investor/governance/[id]` página detalle compartida. Sidebar entries "Gobernanza" para investor + issuer.
- **Fix bug documentos en `/issuer/offerings/new`** (sesión 3): el botón "Subir" en `DocumentsStep` solo flipeaba un boolean sin abrir file picker. Refactor a sub-componente `DocumentRow` con `<input type="file" accept="application/pdf">` oculto + `useRef` para disparar click; tracks filename en `useState` local, muestra al lado del badge, botón "Quitar" para desadjuntar. NO sube a IPFS real todavía (Pinata sin cablear, deuda).
- **Demo flow E2E completo en UI validado** (sesión 3): primer trade real ejecutado entre 2 cuentas distintas a través de la app — TX [`0x0dfa5f15…946b`](https://testnet.snowscan.xyz/tx/0x0dfa5f15a6d49930955db00fa1505681993b90adb04eee2eef08e4177adc946b) en bloque 55442697. Flow completo: `maria.lopez` (wallet alice) firma sell 3 ARKDEMO @ 4.50; `juan.perez` (wallet bob) firma buy 3 ARKDEMO @ 4.80; admin dispara `POST /api/match/execute`; matching engine cruza, SDK avalanche llama `Settlement.executeMatch`, contrato verifica 2 firmas EIP-712, ejecuta swap atómico + fee 0.5%. Balances post-trade verificados: alice 85→82 ARKDEMO + 86.65→100.08 USDC; bob 15→18 ARKDEMO + 933→919.50 USDC; deployer +0.075 USDC fee.
- **Fixes runtime descubiertos en QA E2E**:
  - `/investor/orders` era hardcoded placeholder ("Aún no hay órdenes IFC"). Refactor para usar `useMyOrders` con tabla real, badges de status, botón cancelar para orders open/partial, link a Snowscan del TX hash del trade asociado (no del order hash, que vive solo en event topics).
  - `/api/orders/mine` route **no existía** — hook caía al mock fallback. Creado el endpoint con join orders ↔ trades vía `buyTrades` y `sellTrades` para surface el `lastTradeTxHash` directo en la response.
  - `useWalletBalances` apuntaba a Circle USDC oficial en Fuji en lugar de nuestra MockUSDC. Cambiado a `process.env.NEXT_PUBLIC_USDC`.
  - wagmi config tenía `ssr: true` sin `cookieStorage`/`cookieToInitialState`. Switch a `ssr: false` + localStorage default — más robusto en dev.
  - Zod `addressSchema` transforma a lowercase pero el `link-demo-wallets` guardaba en EIP-55 → Prisma find no match. Corregido a lowercase en el script.
- **Wallet linking de DB para demo**: script `scripts/link-demo-wallets.ts` linkea wallet de alice → `maria.lopez@example.mx` y bob → `juan.perez@example.mx`. Permite que el flow UI de signing funcione con wallets reales KYC'd + funded en Fuji sin pasar por Sumsub completo. Workaround para QA hasta que el flow real de wallet linking de la sesión 2 (SIWE) sea probado por ambos colaboradores.
- **Deuda técnica indexer sync** (parcialmente mitigada): cap_table en DB queda stale tras un trade porque el indexer escucha el mock chain (Redis Streams), no Fuji real. **Mitigado para portfolio**: `portfolio.service.ts` ahora lee balances on-chain directo via viem para cada offering con tokenAddress (no depende de cap_table). Cualquier trade se refleja al instante. Permanente: el indexer debe usar `viem.watchContractEvent` sobre Transfer/TradeExecuted/etc para alimentar cap_table_entries, analytics, dividend allocations, governance snapshots (P0.3 del roadmap). El script `scripts/sync-captable-from-chain.ts` sigue siendo el patch para vistas que aún dependen del cap_table_entries (e.g. la Cap Table page del issuer si no se refactoriza).
- **Portfolio page corregido y refactorizado** (sesión 3): `/investor/portfolio/page.tsx` llamaba `usePortfolio()` sin wallet param (query nunca corría — page vacía para todos). Fix: usa `useWallet()` + pasa address. Además, los PnL hardcoded `$84,320` / `$142,180` que se mostraban idénticos a TODOS los usuarios fueron eliminados — ahora muestran `—` con helper "Pendiente: historial de cost basis" hasta que se implemente trade history per holder. ConnectWalletPrompt agregado para coherencia con `/investor/orders`.
