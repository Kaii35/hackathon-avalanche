...
export function GlobeCdn({
  ...
  // Theme-aware so the globe sits naturally on either app surface.
  // We mount-gate via `mounted` to avoid an SSR/CSR mismatch flicker.
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      globe = createGlobe(canvas, {
        ...
        phi: 0,
        theta: 0.2,
        // Light: warm beige glow against a white surface (original CDN look).
        // Dark : cool brand-blue glow bridges the white sphere to a dark canvas.
        dark: isDark ? 1 : 0,
        ...
        baseColor: [1, 1, 1],
        markerColor: [0, 0, 0],
        glowColor: isDark ? [0.16, 0.22, 0.45] : [0.94, 0.93, 0.91],
    ...
    <div className={`relative aspect-square select-none ${className}`}>
      ...
      {markers.map((m) => (
        <div
          ...
          style={{
            position: 'absolute',
            // CSS Anchor Positioning — Chrome 125+ only. Other browsers
            // ignore positionAnchor and the label renders at the corner,
            // which the var(--cobe-visible-*) opacity gate hides.
      ...
      {traffic.map((t) => (
        <div
          ...
          style={{
            position: 'absolute',
            // CSS Anchor Positioning — Chrome 125+ only (see marker above).
