import { ForcedTransferSchema } from '@hack/shared';
...
export const POST = withErrorHandler(
  withRole(['admin'], async (req, ctx) => {
    ...
    const dto = ForcedTransferSchema.parse(body);
    const result = await adminService.forcedTransfer(ctx.user.email, dto);
